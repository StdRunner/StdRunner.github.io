---
layout: post
title: Docker Compose로 Database 인스턴스 구성 (Postgresql)
date: 2023-01-30
updated: 
tags: [jenkins, docker, postgresql]
menu: ci-cd
---
## 개요
## Docker Compose로 DB 인스턴스 생성, 실행
CI/CD 환경 구성을 위해 Docker Compose를 활용해 DB 인스턴스를 초기화하는 과정이 왜 필요한지에 대한   
개요 및 세부 설명은 앞선 포스트를 참고 바란다. 해당 포스트는 `docker-compose-db.yml` 에 대한 내용과      
`docker-compose-db.yml`을 활용해 Docker Container를 실행하는 내용을 포함하고 있다.

> [Docker Compose로 Database 인스턴스 구성 (개요, Oracle)](https://stdrunner.github.io/2022/12/28/a1-ci-cd-docker-oracle.html)
...
## Postgresql 상세 설정
`docker-compose-db.yml` 파일에서 정의한 Postgresql 서비스 컨테이터의 환경변수 파일 `oracle.env` 내용은 아래와 같다.
##### [oracle.env]
```
